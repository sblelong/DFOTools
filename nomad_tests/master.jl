...
function launch_solar()
    ...
    for seed in seeds
        ...
        param_file = "/Users/sacha/dev/DFOTools/nomad_tests/solar/param.txt"
        (tmppath, tmpio) = mktemp()
        open(param_file) do io
            for line in eachline(io, keep=true)
                if startswith(line, "BB_EXE")
                    line = "BB_EXE \"\$/Users/sacha/dev/solar/bin/solar \$1 \$-seed=$seed\"\n"
                ...
                elseif startswith(line, "STATS_FILE")
                    line = "STATS_FILE \"/Users/sacha/dev/DFOTools/nomad_tests/solar/EB/$seed/stats.txt\" BBE OBJ BBO\n"
                end
                write(tmpio, line)
            end
        end
        ...
        run(`/Users/sacha/dev/nomad/build/release/src/nomad /Users/sacha/dev/DFOTools/nomad_tests/solar/param.txt`; wait=true)
    end
end